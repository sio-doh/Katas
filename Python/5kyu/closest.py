...
# Let us call weight of a number the sum of its digits. For 
# example 99 will have "weight" 18, 100 will have "weight" 
# 1.
# Two numbers are "close" if the difference of their weights 
# is small.
# Task:
# For each number in strng calculate its "weight" and then 
# find two numbers of strng that have:
#     - the smallest difference of weights ie that are the closest
#     - with the smallest weights
#     - and with the smallest indices (or ranks, numbered from 0) 
#     in strng
# Output:
...
# or a pair of two subarrays (Haskell, Clojure, FSharp) or an array of tuples 
# (Elixir, C++)
# or a (char*) in C or a string in some other languages mimicking an array of 
# two subarrays or a string
...
# these weights are different, by their indexes in the string if they have 
# the same weights.
# Examples:
...
def closest(strng):
    ...
    for num in numbers:
        ...
        l.append([total, i, int(num)])
        i += 1
    l.sort()
    a, b = min((l[x][0] - l[x - 1][0], x) for x in range(1, len(l)))
    return [l[b - 1], l[b]]
