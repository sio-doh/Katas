...
# divide a list (an array) of at least two elements into 
# two non-empty parts.
...
#     Elements of a pair must be in the same order as in 
#         the original array.
...
#     "(az, toto picaro zone kiwi)(az toto, picaro zone kiwi)(az toto picaro, zone kiwi)(az toto picaro zone, kiwi)"
# Note
# You can see other examples for each language in 
# "Your test cases"

def partlist(arr):
    ans = []
    for x in range(1, len(arr)):
        ans.append((" ".join(arr[:x]), " ".join(arr[x:]))) 
    return ans
