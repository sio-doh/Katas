...
# book of category A, 114 the sum corresponding to "BKWRK" and 
# "BTSQZ", 50 corresponding to "CDXEF" and 0 to category 'W' since 
# there are no code beginning with W.
# If L or M are empty return string is "" (Clojure/Racket/Prolog 
# should return an empty array/list instead).
# Notes:
# In the result codes and their values are in the same order as in 
# M.
...
def stock_list(list_of_art, list_of_cat): 
    ...
    for art in list_of_art:
        art_temp = art.split(" ")
        if art_temp[0][0] in list_of_cat:
            stock[art_temp[0][0]] = stock.get(art_temp[0][0], 0) + int(art_temp[1])
    ans = ""
    if not stock: return ""
    for cat in list_of_cat: 
        ans += f'({cat} : {stock.get(cat, 0)}) - '
    return ans[:-3]
