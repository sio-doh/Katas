...
# an exception and put entirely into lower case unless it is the 
...
#         always be lowercase except for the first word in the 
#         string.
#     Second argument: the original string to be converted.
# Arguments (Other languages)
#     First argument (required): the original string to be converted.
#     Second argument (optional): space-delimited list of minor words 
#         that must always be lowercase except for the first word in 
#         the string. The JavaScript/CoffeeScript tests will pass 
...
def title_case(title, minor_words=''):
    my_str = ""
    for count, x in enumerate(title.split(" ")):
        if count == 0:
            my_str += " " + x.title()
        else:
            if x.lower() in minor_words.lower().split(" "):
                my_str += " " + x.lower()
            else:
                my_str += " " + x.title()
    return my_str.strip()