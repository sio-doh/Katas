...
skipping any digit with a value of zero. In Roman 
numerals 1990 is rendered: 1000=M, 900=CM, 90=XC; 
resulting in MCMXC. 2008 is written as 2000=MM, 8=VIII; 
or MMVIII. 1666 uses each Roman symbol in descending 
order: MDCLXVI.
Example:
...
Help:
    ...
    D          500
    M          1,000
Remember that there can't be more than 3 identical 
symbols in a row.
...
function solution(number){
    ...
    for (let i = 0; i < dec.length; i++) {
        ...
        if (number >= dec[i]) {
            return roman[i] + solution(number - dec[i]);
        }
    }
}