...
the integers to the right of N. If there is no index 
...
For example:
    ...
    side of the index ({1,2,3}) and the sum of the 
    ...
    of the index ({1}) and the sum of the right side 
    ...
    are equal to 0 in this problem). Index 0 is the 
    place where the left side and right side are equal.

    Note: Please remember that in most 
    programming/scripting languages the index of an 
    array starts at 0.

Input:
    An integer array of length 0 < arr < 1000. The 
    numbers in the array can be any integer positive 
    or negative.
Output:
    The lowest index N where the side to the left of N 
    is equal to the side to the right of N. If you do 
    not find an index that fits these rules, then you 
    will return -1.
Note:
...
function findEvenIndex(arr) { 
    let beginning = 0;
    let ending = arr.reduce((a, b) => a + b, 0); 
