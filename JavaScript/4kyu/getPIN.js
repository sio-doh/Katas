...
function getPINs(observed) {
    ...
    // retrieve all adjacent values for every observed digit
    let alternatives = []; 
    for (const char of observed) {
        alternatives.push(adjacent[char]);
    }
    
    let codes = alternatives[0]; 
    let combinations = [];
    // for each last digit
    for (const digit of alternatives.slice(1)) {
        // for each digit's alternative values 
        for (const alt of digit) {
            // append value to every current combination 
            for (const code of codes) {
                const updatedCode = code + alt; 
                combinations.push(updatedCode);
            }
        }
        // replace codes list with temp list
        codes = combinations; 
        combinations = [];
    }
    return codes.sort();
