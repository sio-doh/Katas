...
have a computer, you can mess with pyramids even if you are not in Egypt 
at the time. For example, let's consider the following problem. Imagine 
...
Your task is to write a function that takes a pyramid representation as 
...
function longestSlideDown(pyramid) {
    return pyramid.reduceRight((end, curr) => curr.map(
        (x, i) => x + Math.max(end[i], end[i+1])
    ))[0];
}